...
// blank assignment to verify that ReconcileConsoleCLIDownload implements reconcile.Reconciler
var _ reconcile.Reconciler = &ReconcileConsoleCLIDownload{}

// ReconcileConsoleCLIDownload reconciles a ConsoleCLIDownload object
type ReconcileConsoleCLIDownload struct {
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileConsoleCLIDownload{client: mgr.GetClient(), scheme: mgr.GetScheme()}
}

// Add creates a new ConsoleCLIDownload Controller and adds it to the Manager.
// The Manager will set fields on the Controller and Start it when the Manager is Started.
func Add(mgr manager.Manager) error {
	return add(mgr, newReconciler(mgr))
}

// add adds a new Controller to manager with r as the reconcile.Reconciler and registers
// the ConsoleCLIDownload type to scheme
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	// register ConsoleCLIDownload to scheme
	r.scheme.AddKnownTypes(consolev1.GroupVersion, &consolev1.ConsoleCLIDownload{})

	// Create a new controller
	c, err := controller.New("consoleclidownload-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource ConsoleCLIDownload
	err = c.Watch(&source.Kind{Type: &consolev1.ConsoleCLIDownload{}}, &handler.EnqueueRequestForObject{}, predicate.GenerationChangedPredicate{})
	if err != nil {
		return err
	}
...
// Reconcile reads that state of the cluster for a kn ConsoleCLIDownload
func (r *ReconcileConsoleCLIDownload) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	reqLogger := log.WithValues("Request.Namespace", request.Namespace, "Request.Name", request.Name)
	reqLogger.Info("Reconciling kn ConsoleCLIDownload")
	err := r.Create()
	if err != nil {
...
func (r *ReconcileConsoleCLIDownload) Create() error {
	if err := createKnDeployment(r.client); err != nil {
		return err
	}

	if err := createCR(r.client); err != nil {
		return err
	}
